        ...
        <div className="min-h-screen py-12 bg-gray-50">
            <div className="max-w-[1700px] mx-auto px-4 sm:px-8 md:px-12 lg:px-20">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-[10px] sm:text-xs text-gray-500 font-bold uppercase tracking-widest">{getTotalItems()} Items in your cart</p>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        {items.map((item) => (
                            <div key={item.id} className="bg-white rounded-3xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all">
                                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                                    {/* Product Image */}
                                    <Link href={`/product/${item.productSlug}`} className="shrink-0 flex justify-center sm:block">
                                        <div className="w-full sm:w-28 sm:h-28 md:w-32 md:h-32 aspect-square max-w-[200px] sm:max-w-none bg-gray-100 rounded-2xl overflow-hidden relative group">
                                            <Image
                                                src={item.image}
                                    ...
                                    <div className="flex-1 space-y-3">
                                        <div>
                                            <Link href={`/product/${item.productSlug}`}>
                                                <h3 className="text-lg sm:text-xl font-black text-gray-900 hover:text-orange-600 transition-colors tracking-tight">
                                                    {item.productName}
                                                </h3>
                                            </Link>
                                            <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-sm font-bold text-gray-500">
                                                {item.size && <span>Size: {item.size}</span>}
                                                {item.color && <span>• Color: {item.color}</span>}
                                                {item.material && <span>• Material: {item.material}</span>}
                                            </div>
                                            <p className="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-wider">SKU: {item.variantSku}</p>
                                        </div>

                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                                            {/* Quantity Controls */}
                                            <div className="flex flex-col gap-1.5 order-2 sm:order-1">
                                                <div className="flex items-center bg-gray-50 border border-gray-200 rounded-full px-1 py-0.5 w-max">
                                                    <button
                                            ...
                                            <div className="flex items-center justify-between sm:justify-end gap-2 order-1 sm:order-2">
                                                <p className="text-xl sm:text-2xl font-black text-gray-900">₹{(item.price * item.quantity).toLocaleString('en-IN')}</p>

                                                <div className="flex items-center gap-1">
                                                    <button
                                                        onClick={() => handleSaveForLater(item.id, item.productId, item.productName)}
                                                        className="p-2.5 hover:bg-rose-50 rounded-full transition-all group/heart text-gray-300"
                                                        title="Save for Later (Moves to Wishlist)"
                                                    >
                                                        <Heart className={`w-5 h-5 transition-colors ${isPending ? 'opacity-50' : 'group-hover/heart:text-rose-500'}`} />
                                                    </button>

                                                    <button
                                                        onClick={() => handleRemove(item.id, item.productName)}
                                                        className="p-2.5 hover:bg-gray-100 rounded-full transition-all group/trash text-gray-300"
                                                        title="Remove from Cart"
                                                    >
                                                        <Trash2 className="w-5 h-5 group-hover/trash:text-red-500 transition-colors" />
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
