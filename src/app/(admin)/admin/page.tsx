    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <Link
            key={index}
            href={stat.href}
            className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between group hover:border-[#2dd4bf] hover:shadow-xl transition-all active:scale-95"
          >
            <div>
            ...
            <div className={`${stat.color} p-4 rounded-2xl text-white shadow-xl group-hover:scale-110 transition-transform`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            <h3 className="font-black text-gray-900 uppercase italic tracking-tighter mb-6 flex items-center gap-3">
              <Package className="w-5 h-5 text-rose-500" /> Low Stock Inventory
            </h3>
            <div className="space-y-4">
              {statsData?.lowStockProducts && statsData.lowStockProducts.length > 0 ? (
                statsData.lowStockProducts.map((product: any) => (
                  <div key={product.id} className="flex items-center justify-between p-4 rounded-2xl bg-rose-50/30 border border-rose-100 group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-rose-600 uppercase tracking-widest">
                          {product.variants.reduce((sum: number, v: any) => sum + v.stock, 0)} units remaining
                        </p>
                      </div>
                    </div>
                    <Link href={`/admin/products/${product.id}/edit`}>
                      <button className="bg-white text-gray-900 border border-gray-200 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all">
                        Restock
                      </button>
                    </Link>
                  </div>
